...
const { calculateSessions, numSessions, medianSessionsLength, numUniqueVisitedSites } = require( '../services/SiteVisitsManager' );

router.get( '/calcSessions', async ( req, res ) => {
    try {
        console.log( `api.js/calcSessions: new Req for sessions calculation` );
        await calculateSessions();
...
router.get( '/numSessions/:siteUrl', async ( req, res ) => {
    ...
    try {
        ...
        const num = await numSessions( siteUrl );
...
router.get( '/medianSessionsLength/:siteUrl', async ( req, res ) => {
    ...
    try {
        ...
        const medSessionLength = await medianSessionsLength( siteUrl );
        res.status( 200 ).send( `Median session length for site ${ siteUrl } = ${ medSessionLength } ` );
...
router.get( '/numUniqueVisitedSites/:visitorId', async ( req, res ) => {
    ...
    try {
        ...
        const uniqueVisitedSitesNum = await numUniqueVisitedSites( visitorId );
        res.status( 200 ).send( `Num of unique sites for ${ visitorId } = ${ uniqueVisitedSitesNum } ` );
